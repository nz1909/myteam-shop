...
public class CartInfo implements Serializable {
    ...
    private List<TCommodity> productList;// 购物车中商品列表
    ...
    public List<TCommodity> getProductList() {
        if(productList==null){
            productList = new LinkedList<TCommodity>();
        }
        return productList;
    }

    public void setProductList(List<TCommodity> productList) {
    ...
    public void totalCacl(){
        ...
        for(int i=0;i<getProductList().size();i++){
            TCommodity p = getProductList().get(i);
